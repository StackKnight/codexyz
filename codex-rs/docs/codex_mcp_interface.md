...
- Server binary: `codexyz mcp-server` (or `codex-mcp-server`)
...
```bash
codexyz mcp-server | your_mcp_client
```

For a simple inspection UI, you can also try:

```bash
npx @modelcontextprotocol/inspector codexyz mcp-server
```

Use the separate `codexyz mcp` subcommand to manage configured MCP server launchers in `config.toml`.
