//! Implements the `codexyz doctor` diagnostic report.
//!
...
///
/// This is the CLI entry point for codexyz doctor. It does not repair issues;
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun codexyz doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
...
/// JSON support report emitted by `codexyz doctor --json`.
///
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    if path_entries.len() > 1 {
        details.push(format!("PATH codexyz entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH codexyz #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary = "npm install -g codexyz would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Codexyz so the JS shim provides CODEX_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("codexyz");
...
fn codex_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["codexyz"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "codexyz"]);
    ...
    match load_auth_dot_json(&config.codex_home, config.cli_auth_credentials_store_mode) {
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check = check
                    .remediation("Run codexyz login again or provide a supported auth env var.");
            }
            check
        }
        ...
        .details(details)
        .remediation("Run codexyz login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run codexyz login again."),
    }
}
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/codexyz");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("codexyz"),
            }
        );
    }
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            codex_self_exe: Some(PathBuf::from("/bin/codexyz")),
