...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run codexyz doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
Notes
   ...
   ✗ auth         token expired - Run `codexyz login`.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `codexyz login`.
-------------------------------------------------------------
...
",
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/codex/target/debug/codexyz · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/codex/target/debug/codexyz"));
