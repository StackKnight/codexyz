...
pub struct Cli {
    ...
    // Internal controls set by the top-level `codexyz resume` subcommand.
    // These are not exposed as user flags on the base `codexyz` command.
    #[clap(skip)]
    ...
    /// top-level `codexyz resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `codexyz fork` subcommand.
    // These are not exposed as user flags on the base `codexyz` command.
    #[clap(skip)]
    ...
    /// top-level `codexyz fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
