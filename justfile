...
alias c := codexyz
codexyz *args:
    cargo run --bin codexyz -- {args}
# `codexyz exec`
exec *args:
    cargo run --bin codexyz -- exec {args}
# Start `codexyz exec-server` and run codex-tui.
[no-cd]
...
app-server-test-client *args:
    ...
    cargo run -p codex-app-server-test-client -- --codex-bin ./target/debug/codexyz {args}
...
bench-smoke:
    just bench -- --test
# Build and run Codexyz from source using Bazel.
# On Unix, use `[no-cd]` and `--run_under="cd $PWD &&"` to ensure Bazel runs
# the command in the current working directory.
[no-cd]
[unix]
bazel-codexyz *args:
    bazel run //codex-rs/cli:codexyz --run_under="cd $PWD &&" -- "$@"
[windows]
bazel-codexyz *args:
    bazel run //codex-rs/cli:codexyz --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)
[no-cd]
bazel-lock-update:
