...
function Assert-ValidReleaseVersion {
    ...
    if ($Version -cne "latest" -and $Version -cnotmatch "^[0-9]+\.[0-9]+\.[0-9]+(?:-(?:alpha|beta)(?:\.[0-9]+)?)?$") {
        throw "Invalid Codexyz release version: $Version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]."
    }
}
...
function Find-ReleaseAssetMetadata {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/StackKnight/codexyz/releases/tags/rust-v$ResolvedVersion"
...
function Get-ReleaseAssetMetadata {
    ...
    if ($null -eq $metadata) {
        throw "Could not find release asset $AssetName for Codexyz $ResolvedVersion."
    }
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded Codexyz archive checksum did not match expected digest. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Resolve-Version {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/StackKnight/codexyz/releases/latest"
    if (-not $release.tag_name) {
        Write-Error "Failed to resolve the latest Codexyz release version."
        exit 1
    }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -CodexPath (Join-Path $StandaloneCurrentDir "bin\codexyz.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }

    $standaloneVersion = Get-VersionFromBinary -CodexPath (Join-Path $StandaloneCurrentDir "codexyz.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older Codexyz install at $VisibleBinDir"
    Write-WarningStep "To continue, Codexyz needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current Codexyz setup now?")) {
...
function Test-ReleaseIsComplete {
    ...
    switch ($Layout) {
        ...
        default {
            throw "Unknown Codexyz installer layout: $Layout"
        }
    }
...
function Get-ExistingCodexCommand {
    $existing = Get-Command codexyz -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-ConflictingInstall {
    ...
    Write-Step "Detected existing $manager-managed Codexyz at $existingPath"
    Write-WarningStep "Multiple managed Codexyz installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    if (Prompt-YesNo "Uninstall the existing $manager-managed Codexyz now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed Codexyz. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed Codexyz installed. PATH order will determine which codexyz runs."
    }
}
...
function Test-VisibleCodexCommand {
    ...
    $codexCommand = Join-Path $VisibleBinDir "codexyz.exe"
    & $codexCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed Codexyz command failed verification: $codexCommand --version"
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "Codexyz requires a 64-bit version of Windows."
    exit 1
}
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating Codexyz CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating Codexyz CLI"
} else {
    Write-Step "Installing Codexyz CLI"
}
...
if ($null -eq $packageMetadata -or $null -eq $checksumMetadata) {
    ...
    if ($null -ne $packageMetadata) {
        ...
        throw "Could not find Codexyz package or platform npm release assets for Codexyz $resolvedVersion."
    }
    $checksumMetadata = $null
}
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("codexyz-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            ...
            Write-Step "Downloading Codexyz CLI"
            ...
            if ($installLayout -eq "Package") {
                ...
                if (-not (Test-PackageContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded Codexyz package archive did not contain the expected package layout."
                }
            } else {
                ...
                if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded Codexyz npm archive did not contain the expected legacy platform package layout."
                }
            }
...
Write-Step "Current PowerShell session: codexyz"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: codexyz"
Write-Host "Codexyz CLI $resolvedVersion installed successfully."

$codexCommand = Join-Path $visibleBinDir "codexyz.exe"
if (Prompt-YesNo "Start Codexyz now?") {
    Write-Step "Launching Codexyz"
    & $codexCommand
}
