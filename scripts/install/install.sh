...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-(alpha|beta)(\.[0-9]+)?)?$'; then
    echo "Invalid Codexyz release version: $version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]." >&2
    exit 1
  fi
}
...
download_file() {
  ...
  echo "curl or wget is required to install Codexyz." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/StackKnight/codexyz/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/StackKnight/codexyz/releases/tags/rust-v%s\n' "$resolved_version"
}
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Codexyz download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Codexyz archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Codexyz." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/StackKnight/codexyz/releases/latest")"
  resolved="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"rust-v\([^"]*\)".*/\1/p' | head -n 1)"

  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest Codexyz release version." >&2
    exit 1
  fi
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 \( -name '.codex.*' -o -name '.codexyz.*' \) -exec rm -f {} +
  fi
}
...
current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/codexyz" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi

  version="$(version_from_binary "$CURRENT_LINK/codexyz" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_codex() {
  command -v codexyz 2>/dev/null || true
}
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && codexyz"
      step "Future terminals: open a new terminal and run: codexyz"
...
maybe_launch_codex_now() {
  if prompt_yes_no "Start Codexyz now?"; then
    step "Launching Codexyz"
    "$BIN_PATH"
  fi
}
...
detect_conflicting_install() {
  ...
  step "Detected existing $manager-managed Codexyz at $existing_path"
  warn "Multiple managed Codexyz installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask codexyz"
      ;;
    bun)
      uninstall_cmd="bun remove -g codexyz"
      ;;
    *)
      uninstall_cmd="npm uninstall -g codexyz"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Codexyz now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Codexyz. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Codexyz installed. PATH order will determine which codexyz runs."
  fi
}
...
install_package_release() {
  ...
  chmod 0755 "$stage_release/bin/codexyz" "$stage_release/codex-path/rg"
  if [ -f "$stage_release/codex-resources/bwrap" ]; then
    chmod 0755 "$stage_release/codex-resources/bwrap"
  fi
  ln -sf "bin/codexyz" "$stage_release/codexyz"
...
install_legacy_platform_npm_release() {
  ...
  cp "$vendor_root/bin/codexyz" "$stage_release/codexyz"
  cp "$vendor_root/codex-path/rg" "$stage_release/codex-resources/rg"
  chmod 0755 "$stage_release/codexyz" "$stage_release/codex-resources/rg"
  ...
  case "$layout" in
    package)
      [ -f "$release_dir/codex-package.json" ] &&
        [ -x "$release_dir/bin/codexyz" ] &&
        [ -x "$release_dir/codexyz" ] &&
...
release_codex_relative_path() {
  ...
  if [ -x "$release_dir/bin/codexyz" ]; then
    printf 'bin/codexyz\n'
  else
    printf 'codexyz\n'
  fi
...
if release_asset_exists "$package_asset" "$resolved_version" &&
  ...
  echo "Could not find Codexyz package or platform npm release assets for Codexyz $resolved_version." >&2
  exit 1
fi
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Codexyz CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Codexyz CLI"
else
  step "Installing Codexyz CLI"
fi
...
printf 'Codexyz CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_codex_now
